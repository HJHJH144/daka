...
SIGN_TIMESTAMP_OFFSET_MS = int(os.getenv("SIGN_TIMESTAMP_OFFSET_MS", "50000"))

app = Flask(__name__)
FRONTEND_ORIGIN = os.getenv("FRONTEND_ORIGIN", "https://lczcl.xyz")
...
def sign_course(user_id: str, session_id: str, course_sched_id: str) -> Tuple[bool, Dict[str, Any]]:
    params = {"id": user_id}
    headers = {"sessionId": session_id}
    timestamp = int(time.time() * 1000) + SIGN_TIMESTAMP_OFFSET_MS
    ...
    if not resp.ok:
        audit_logger.info(
            "签到上游失败 userId=%s hasSessionId=%s courseSchedId=%s httpStatus=%s",
            user_id,
            bool(session_id),
            course_sched_id,
            resp.status_code,
    ...
    sign_flag = _extract_sign_success_flag(data)

    if status_text == "0" and sign_flag is not False:
        return True, {
            "message": "打卡成功",
            "httpStatus": resp.status_code,
            "status": status_text,
    ...
    detail = err_msg or "打卡失败"
    if status_text == "0" and sign_flag is False and not err_msg:
        detail = "签到状态未成功"
    if err_code:
        detail = f"{detail}(ERRCODE={err_code})"
    audit_logger.info(
        "签到业务失败 userId=%s hasSessionId=%s courseSchedId=%s status=%s errCode=%s errMsg=%s",
        user_id,
        bool(session_id),
        course_sched_id,
        status_text or "-1",
